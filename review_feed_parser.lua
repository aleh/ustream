...
--[[
    ...
    local parser = require("review_feed_parser")(
		function(p, review)
            ...
            -- Called after the parsing has completed successfully
        end,
        error = function(p, message)
            -- Calling after the parsing has failed.
        end
    )
	
	(Note that similar to JSON parser we don't have a 'new' method nor accept a table of parameters anymore to free up
	a few bytes of RAM.)
...
return function(review_callback, done_callback, error_callback)
